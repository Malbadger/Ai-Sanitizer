...
function loadRules() {
    chrome.storage.local.get('sanitize_rules', (data) => {
        let rules = data.sanitize_rules;
        if (!rules || rules.length === 0) {
            rules = DEFAULT_RULES;
            chrome.storage.local.set({ sanitize_rules: rules }, () => {
                console.log('Initialized with default rules:', rules);
            });
        }
...
function resetDefaults() {
    if (confirm('Are you sure you want to reset to default rules? This will overwrite all existing rules.')) {
        chrome.storage.local.set({ sanitize_rules: DEFAULT_RULES }, () => {
            loadRules();
...
function clearAllRules() {
    if (confirm('Are you sure you want to clear all rules? This action cannot be undone.')) {
        chrome.storage.local.remove('sanitize_rules', () => {
            loadRules();
...
function loadToggleState() {
    chrome.storage.local.get('sanitizationEnabled', (data) => {
        ...
        if (toggle && toggleSlider) {
            ...
            console.error('Toggle input #enabled or .toggle-slider not found in DOM');
        }
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (toggle && toggleSlider) {
        toggle.addEventListener('change', function() {
            ...
            chrome.storage.local.set({ sanitizationEnabled: isEnabled }, (result) => {
                if (chrome.runtime.lastError) {
                    ...
                    console.log('Sanitization Enabled state saved:', isEnabled);
                }
            });
        });
    } else {
        console.error('Toggle input #enabled or .toggle-slider not found when attaching event listener');
    }
...
console.log("Options page script loaded, Rule Management Enabled.");