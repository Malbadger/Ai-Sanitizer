    ...
    browserApi.storage.local.get('sanitize_rules', (data) => {
        ...
        if (!rules || rules.length === 0) {
            ...
            browserApi.storage.local.set({ sanitize_rules: rules }, () => {
                console.log('Initialized with default rules (Edge):', rules);
            });
        }
...
function loadToggleState() {
    ...
    browserApi.storage.local.get('sanitizationEnabled', (data) => {
        ...
        if (toggle && toggleSlider) {
            ...
            console.log('Toggle state loaded (Edge):', isEnabled);
        } else {
            console.error('Toggle input #enabled or .toggle-slider not found in DOM (Edge)');
        }
    });
}
        ...
        toggle.addEventListener('change', function() {
            ...
            browserApi.storage.local.set({ sanitizationEnabled: isEnabled }, (result) => {
                if (browserApi.runtime.lastError) {
                    console.error('Storage set failed (Edge):', browserApi.runtime.lastError);
                    alert('Failed to save toggle state. Check console for details.');
                } else {
                    console.log('Sanitization Enabled state saved (Edge):', isEnabled);
                }
            });
        });
    } else {
        console.error('Toggle input #enabled or .toggle-slider not found when attaching event listener (Edge)');
    }
...
console.log("Options page script loaded (Edge), Rule Management Enabled.");